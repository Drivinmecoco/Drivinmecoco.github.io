...
const flip = function (playerBoard) {
    const rowFlipped = playerBoard.map((row) => row.slice(0).reverse());
    const columnFlipped = rowFlipped.slice(0).reverse(0);
...
const defaultPlayerBoard = function () {
    const rowOne = [0, 1, 0, ...new Array(5).fill(0)];
    const rowTwo = [1, 0, 1, ...new Array(5).fill(0)];
...
/**
 * A function that checks if a playerboard is empty.
* @function
* @param {Checkers.PlayerBoard}
* @return {bool}
*/
 ...
 * @function
 * @param {Checkers.Board} board
 * @returns {number} -1 no one, 0 player0 won, 1 player1 won
 */
